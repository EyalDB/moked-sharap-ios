...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let schemeUrl = connectionOptions.urlContexts.first?.url {
            // Check if this is a call request from the widget
            if schemeUrl.scheme == "mokadsharap" && schemeUrl.host == "call" {
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
        if let scheme = URLContexts.first?.url {
            // Check if this is a call request from the widget
            if scheme.scheme == "mokadsharap" && scheme.host == "call" {
                // Trigger haptic feedback
                let generator = UIImpactFeedbackGenerator(style: .heavy)
                generator.impactOccurred()
            ...
            if let url = comps?.url {
                ...
                מוקדשרפ.webView.evaluateJavaScript("location.href = '\(url)'")
            }
        }
    }
    ...
    func scene(_ scene: UIScene, continue userActivity: NSUserActivity) {
        ...
        // Handle it inside our web view in a SPA-friendly way.
        מוקדשרפ.webView.evaluateJavaScript("location.href = '\(universalLink)'")
    }
    ...
    func windowScene(_ windowScene: UIWindowScene,
        ...
        if let shortcutUrl = URL.init(string: shortcutItem.type) {
            מוקדשרפ.webView.evaluateJavaScript("location.href = '\(shortcutUrl)'");
        }
    }
